...
{
    public ArmorPlated(String pInternalName, int pArmorPart) {
    ...
    {
        for(ArmorMaterialMedieval tMaterial: ARegistry.iInstance.getArmorMaterials().values()){
            ItemStack tStandardArmor = MedievalArmorFactory.iInstance.buildNewMLAArmor(this, new HashMap<MLAAddon, Integer>(), tMaterial.getBaseDurability(this.getInternalName()), tMaterial.iInternalName);
    ...
    {
        if (!(pNewAddon instanceof ArmorModifierPlated))
        {
            throw new InvalidParameterException("The given addon is not allowed on plated armor.");
        }
